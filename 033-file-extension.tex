...
\hlstd{knit_hooks}\hlopt{$}\hlkwd{set}\hlstd{(}\hlkwc{plot} \hlstd{=} \hlkwa{function}\hlstd{(}\hlkwc{x}\hlstd{,} \hlkwc{options}\hlstd{) \{}
    \hlstd{x} \hlkwb{=} \hlkwd{paste}\hlstd{(x,} \hlkwc{collapse} \hlstd{=} \hlstr{"."}\hlstd{)}  \hlcom{# x is file.ext now instead of c(file, ext)}
    \hlkwd{paste0}\hlstd{(}\hlstr{"\textbackslash{}\textbackslash{}end\{kframe\}"}\hlstd{,} \hlkwd{hook_plot_tex}\hlstd{(x, options),} \hlstr{"\textbackslash{}\textbackslash{}begin\{kframe\}"}\hlstd{)}
\hlstd{\})}
\end{alltt}
\end{kframe}
\end{knitrout}
