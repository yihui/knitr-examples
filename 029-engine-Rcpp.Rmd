...
The above example defines a single C++ function by calling `cppFunction` (which automatically includes the `Rcpp.h` header). If you want to define multiple functions (or helper functions that are not exported) then you need to add the appropriate includes and `Rcpp::export` attributes (in this case the `sourceCpp` function will be called). For example:
...
Finally, you can also use the abbreviated form of single function definition with an external package by using the `engine.opts` option, e.g. we use the **RcppArmadillo** package below:
