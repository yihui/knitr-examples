...
exists <- nchar(where) > 0 # Only run chunk if runtime exists
```
...
```{r something-in-R, engine='R', comment='R>', eval=exists[1]}
...
```{r something-in-Scala, engine='scala', comment='Scala>', engine.opts='-savecompiled', cache=TRUE, eval=exists[3]}
...
```{r something-in-Python, engine='python', comment='Python>', eval=exists[2]}
import os
...
```{r something-in-Bash, engine='bash', comment='Bash>', eval=exists[4]}
something=$SOMETHING
...
```{r something-in-Perl, engine='perl', comment='Perl>', eval=exists[5]}
